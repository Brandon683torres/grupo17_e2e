...
Given("I login with {kraken-string} and {kraken-string} - Escenario 15", async function (email, password) {
    await login(this.driver, email, password);
    await screenCapture(this.driver, "Escenario15", "login");
});

When("I click on the Members tab - Escenario 15", async function () {
    await clickOnMembersTab(this.driver);
    await screenCapture(this.driver, "Escenario15", "clickOnMembersTab");
});

When("I click on the Filter Tool", async function () {
    await clickOnFilterTool(this.driver);
    await screenCapture(this.driver, "Escenario15", "clickOnFilterTool");
});

When("I write the Filter", async function () {
    await writeOnFilterTool(this.driver);
    await screenCapture(this.driver, "Escenario15", "writeOnFilterTool");
});

When("I apply the Filter", async function () {
    await applyFilterTool(this.driver);
    await screenCapture(this.driver, "Escenario15", "applyFilterTool");
});
